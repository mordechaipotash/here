...
interface RawClient {
  ...
  percentage: string;
  states_company_registered: string;
...
function parseBoolean(value: string): boolean {
  if (!value || value.toLowerCase() === 'nan') return false;
...
async function importClients() {
  ...
  const clientsData: RawClient[] = await new Promise((resolve, reject) => {
    const results: RawClient[] = [];
    fs.createReadStream(clientsFile)
      .pipe(csv.parse({ columns: true, skip_empty_lines: true }))
      .on('data', (data: RawClient) => {
        // Clean up the data
...
async function importEmailDomains() {
  ...
  const domainsData: EmailDomain[] = await new Promise((resolve, reject) => {
    const results: EmailDomain[] = [];
    fs.createReadStream(domainsFile)
      .pipe(csv.parse({ columns: true, skip_empty_lines: true }))
      .on('data', (data: EmailDomain) => {
        // Clean up the data
        Object.keys(data).forEach(key => {
          if (typeof data[key] === 'string') {
            data[key] = data[key].trim();
  ...
  const clientMap = new Map(
    clients?.map(c => [normalizeClientName(c.client_name), c.client_id]) || []
  );

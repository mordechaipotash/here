...
async function getEmailStats(): Promise<EmailStats[]> {
  ...
  try {
    ...
    const stats: EmailStats[] = Array.from(emailMap.entries())
      .map(([email, stats]) => ({
        email,
        domain: stats.domain,
...
async function writeStatsToCSV(stats: EmailStats[], outputPath: string) {
  console.log('Writing statistics to CSV...');
  
  const csvContent = [
    'email,domain,frequency,first_seen,last_seen',
    ...stats.map(stat => 
      `"${stat.email}","${stat.domain}",${stat.frequency},"${stat.first_seen}","${stat.last_seen}"`)
  ].join('\n');

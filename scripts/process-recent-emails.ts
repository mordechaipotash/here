...
async function findClientByDomain(email: string): Promise<{ clientId: string | null, domain: string | null }> {
    ...
    for (const messageResponse of messageDetails) {
      ...
      const { data: emailData, error: emailError } = await supabase
        ...
        .insert({
          ...
          processed: false,
          track: 'forms_admin', // Default to forms_admin, will update to wotc_machine if PDF found

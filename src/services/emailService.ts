...
export class EmailService {
  static async getEmails(filter?: EmailFilter): Promise<{ data: Email[]; count: number }> {
    let query = supabase
      .from('email_view')
      .select('*', { count: 'exact' });
  ...
  static async getEmailById(emailId: string): Promise<Email | null> {
  ...
  static async updateEmailStatus(emailId: string, status: Email['status']): Promise<void> {
